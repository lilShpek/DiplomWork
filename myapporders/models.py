...
from myappproducts.models import Product
...
class Order(models.Model):
    ...
    def __str__(self):
        ...
        if self.status:
            return f"Заказ №{self.id} от {self.customer_name} ({self.total_price}р.) — {self.status.name}"
        else:
            
            return f"Заказ №{self.id} от {self.customer_name} ({self.total_price}р.) — Статус не указан"
